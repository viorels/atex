...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # XXX: this code assumes that the sets of id and ancora_id do not intersect
        for product in orm['atex_web.product'].objects.all():
            hits = [{'count': hit.count,
                     'date': hit.date} for hit in product.hit_set.all()]
            product.delete()
            product.id = product.ancora_id
            product.save(force_insert=True)
            for hit in hits:
                product.hit_set.create(**hit)

    def backwards(self, orm):
        for product in orm['atex_web.product'].objects.all():
            product.ancora_id = product.id
            product.save()
